...
SELECT category, AVG(rating) as average_rate FROM analytics GROUP BY category ORDER BY avg(rating) desc;
...
SELECT app_name,rating,reviews FROM analytics WHERE reviews > 10000 AND rating < 3; 
...
SELECT app_name,reviews,min_installs,ROUND(min_installs/reviews,2) as proportion FROM analytics WHERE min_installs > 100000 ORDER BY proportion desc LIMIT 1;